...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var chicagoDeptNo = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo);
        var result = emps.Where(e => chicagoDeptNo.Contains(e.DeptNo)).ToList();
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.Where(e => e.Sal > emps.Where(emp => emp.DeptNo == e.DeptNo)
                                              .Average(emp => emp.Sal))
                        .Select(e => e.EName)
                        .ToList();
